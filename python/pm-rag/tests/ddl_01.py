...
insert_simulated_data: list[str] = [
    """
    SELECT '----- Ordem 1: Falha Corretiva (Vibração alta - 1 Chunk) -----' AS description;
    """,
    """
    ...
    """,
    """
    SELECT '----- Ordem 2: Manutenção Preventiva (Inspeção de rotina - 1 Chunk) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- Ordem 3: Falha Emergencial (Perda de pressão - 1 Chunk) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- Ordem 4: Documentação Técnica (Manual de Calibração - 2 Chunks > 500, > 750) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- Ordem 5: Relatório de Longa Duração (Desgaste e Vida Útil - 3 Chunks > 1150) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- Ordem 6-50 (45 INSERTS): Combinação de Curto, Médio e Longo. s functional_location e equipment_number são cruciais para o RAG correlacionar com as TAGs -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- 15 CHUNKS de tamanho > 500 (incluindo > 750 e > 1150 já inseridos) -----' AS description;
    """,
    """
    SELECT '----- OM 40016016 - Manutenção Corretiva (2 Chunks, 1 > 500) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- OM 40017017 - Falha de Instrumentação (1 Chunk > 500). Tamanho Aprox: 850 ( > 750) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ('40016016', 'BOMBA_DRENO_22', 'B-22-A', 'PM01',
        ...
        2, generate_random_embedding(512), '2024-10-18');

    --
    """,
    """
    SELECT '----- OM 40018018 - Falha de Potência (2 Chunks, 1 > 750) -----' AS description;
    """,
    """
    SELECT '----- Tamanho Aprox: 700 (> 500) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ('40017017', 'SISTEMA_INSTRUMENTACAO', 'SI-T05', 'PM01',
        ...
        1, generate_random_embedding(512), '2024-10-17');
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- Tamanho Aprox: 950 ( > 750) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ('40018018', 'DISJUNTOR_PRINCIPAL', 'DJ-M01', 'PM01',
        ...
        2, generate_random_embedding(512), '2024-10-16');
    """,
    """
    SELECT '----- OM 60019019 - Revisão de Procedimento (1 Chunk > 1150) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- 35 INSERTS adicionais (misturando tipos PM01/PM03, tamanhos e localizações) -----' AS description;
    """,
    """
    SELECT '----- Omitindo texto longo para brevidade, mas o texto é significativo para o RAG. -----' AS description;
    """,
    """
    SELECT '----- Curto ( < 500) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- Médio (entre 500 e 750) - 5 Chunks > 500 adicionais -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
    ...
    """,
    """
    SELECT '----- Inserção dos 35 comandos restantes (curtos) para atingir 50 registros no total (IDs 40030030 a 40049049) (25 Curto/Médio) -----' AS description;
    """,
    """
    ...
    SELECT
        ...
        1, generate_random_embedding(512), ('2024-10-' || LPAD((s.i % 20 + 1)::text, 2, '0'))::DATE
    FROM generate_series(30, 49) AS s(i);
    """,
    """
    SELECT '----- Inserção de mais comandos de exemplo (PM02) -----' AS description;
    """,
    """
    INSERT INTO sap_pm_rag_data (sap_order_id, functional_location, equipment_number, order_type, maintenance_text, chunk_id, embedding, creation_date) VALUES
